...
class ScienceAgent():
    def __init__(self, llm_engine_name, context_cutoff=28000, use_self_debug=False, use_knowledge=False, logs_dir=None):
        self.llm_engine = LLMEngine(llm_engine_name)
        self.llm_cost = model_cost.get(llm_engine_name, {"input_cost_per_token": 0.0, "output_cost_per_token": 0.0})
    ...
    def _save_full_response(self, full_response, out_fname, step=0):
        """Save the full model response (including thinking) to the logs directory."""
        if self.logs_dir is None or full_response is None:
            return
        # Derive log filename from the program filename
        program_name = Path(out_fname).stem  # e.g. "pred_3k"
        suffix = f"_step{step}" if step > 0 else ""
        log_fname = Path(self.logs_dir, f"{program_name}{suffix}.txt")
    ...
    def write_program(self, assistant_output, out_fname):
        ...
        if match:
            ...
            # If no markdown block found, check if the entire response looks like Python code
            # (starts with import, def, class, #, or is multi-line code)
            stripped = assistant_output.strip()
    ...
    def step(self, out_fname, output_fname, step_num=0):
        ...
        if (not special_err) and exec_res.returncode == 0:
            ...
            assistant_output, prompt_tokens, completion_tokens, full_response = self.llm_engine.respond(user_input, temperature=0.6, top_p=0.95)
            ...
            self._save_full_response(full_response, out_fname, step=step_num)
    ...
    def solve_task(self, task, out_fname):
        ...
        self._save_full_response(full_response, out_fname)
        ...
        if self.use_self_debug:
            for t in range(10):
                halt, new_cost = self.step(out_fname, task["output_fname"], step_num=t+1)
                cost += new_cost
                if halt:
                    break
