...
# Configuration
# ============================================
MODEL_PATH="/fs/ess/PAA0201/hananemoussa/autosdt-v2/saves/qwen3-4B-instruct-full-sft"
PORT=${PORT:-8001}
NUM_GPUS=2
MAX_MODEL_LEN=16384
...
if [ "$RUN_SERVER_ONLY" = false ]; then
    ...
    srun --account=PAS3019 \
         ...
         --mem=64G \
         --time=2:00:00 \
         --pty bash -c "cd /fs/ess/PAA0201/hananemoussa/my-SAB && bash start_vllm_qwen3_4b_instruct_full_interactive.sh run_server_only"
...
export HF_HOME=/users/PAA0201/hananemoussa/.cache/huggingface
mkdir -p $HF_HOME
